...
离线评测固定使用 21 个 scripted-model 场景驱动生产 `StateGraph`、节点、ToolNode、scheduler、路由、证据门禁、隔离工作区和 HITL，不创建供应商客户端、不读取 `.env`、不调用付费 API。`expected` 只负责运行后的断言，不能进入 actual 生成路径；基准只保存归一化状态和用量，不保存 Prompt、源码全文、模型原始响应或凭据。

| 指标 | 当前离线基准 | 含义 |
| --- | ---: | --- |
| 场景通过率 | 21/21（100%） | 每个场景均到达预期终态并通过行为断言 |
| 行为检查通过率 | 210/210（100%） | 结构化安全与控制面断言全部通过 |
| 首次审查成功率 | 64.29% | 需要审查的成功路径中首次通过比例 |
| 最终重构成功率 | 38.10% | 包含故意失败、安全阻断、事务冲突和用户拒绝场景，因此不是总质量分 |
| 平均重试次数 | 0.4286 | 每场景平均 Reviewer/任务重试 |
| 安全策略阻断率 | 100% | 预期被阻断的攻击场景全部被阻断 |
| HITL 拒绝率 | 9.09% | 基准中设计为用户拒绝的审批占比 |
